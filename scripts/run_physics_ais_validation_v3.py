...
def run_v3_validation():
    ...
    print("=" * 80)
    print(" GEODESIC PHYSICS & AIS VALIDATION SUITE (v3)")
    ...
    print(f"  Statistical Conclusion:                p = {p_empirical:.4f} indicates the ranking provides investigative")
    ...
    with open(os.path.join(OUT_DIR, "validation_summary_v3.json"), "w") as f:
    ...
    plt.tight_layout()
    fig_path = os.path.join(OUT_DIR, "geodesic_physics_ais_validation_panel_v3.png")
    plt.savefig(fig_path, dpi=300)
    plt.close()

    print(f"\n[✓] Validation Panel Figure Saved -> {fig_path}")
    print(f"[✓] Validation Master Summary Saved -> {os.path.join(OUT_DIR, 'validation_summary_v3.json')}")
    print("=" * 80 + "\n")
